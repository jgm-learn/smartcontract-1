...
library LibTradeMap
{
    ...
    function insert(TradeMap storage self, uint k, uint trade_date, bytes32 opp_user_id, bytes32 bs, uint confirm_qty, uint fee, bytes32 trade_state, StructMarket.value storage market_value) internal returns (bool replaced)
    {
        replaced = true;
        if(self.data[k].user_id_ == "")
        {
        ...
        self.data[k].user_id_ = market_value.user_id_;//TODO 根据买卖标志取自己的id
